...
import{d as X,y as Y,q as $,_ as Z,A as tt,c as v,a as c,h as C,t as y,I as nt,f as H,g as I,w as V,Q as j,b as et,j as l,r as it,n as ht,o as f}from"./index-BMIcMLSK.js";import{_ as ct}from"./ThemeToggle.vue_vue_type_script_setup_true_lang-DLy1SSEM.js";import{u as ot}from"./useStudyClock-lkvGDx-q.js";const rt=`<h2 class="dom">Phase 1 — Programming Fundamentals</h2>
...
`,at={class:"notes-interview"},dt={class:"layout"},lt={class:"search"},gt={key:0,class:"prog"},st={class:"bar"},bt={key:0,class:"bad"},pt={key:1},ut={class:"topbar"},mt={key:0,class:"nohit"},Lt=X({__name:"InterviewNotesPage",setup(kt){ot("interview","notes");const F=ht(),T=Y("interview"),g=l(),_=l(),s=l(),x=l(!1),M=l(0),p=l(0),L=l(0),q=l(0);let w=[],u=[],m=null,k=null,N,P=null;function U(){const n=[];g.value.querySelectorAll("h2.dom, section > h3").forEach(t=>{var e;if(t.tagName==="H2")n.push(`<div class="grp">${t.textContent}</div>`);else{const i=t.closest("section"),o=(((e=t.firstChild)==null?void 0:e.textContent)||t.textContent||"").trim();n.push(`<a href="#${i.id}">${o}</a>`)}}),_.value.innerHTML=n.join(""),u=[..._.value.querySelectorAll("a")],u.forEach(t=>t.addEventListener("click",e=>{e.preventDefault(),location.hash=t.getAttribute("href")}))}const Q=n=>{n.querySelectorAll("mark").forEach(t=>t.replaceWith(document.createTextNode(t.textContent||""))),n.normalize()};function W(n,t){const e=document.createTreeWalker(n,NodeFilter.SHOW_TEXT),i=[];let o;for(;o=e.nextNode();)(o.nodeValue||"").toLowerCase().includes(t)&&i.push(o);i.forEach(b=>{const h=document.createDocumentFragment(),a=b.nodeValue||"",A=a.toLowerCase();let d=0,r;for(;(r=A.indexOf(t,d))>-1;){h.appendChild(document.createTextNode(a.slice(d,r)));const K=document.createElement("mark");K.textContent=a.slice(r,r+t.length),h.appendChild(K),d=r+t.length}h.appendChild(document.createTextNode(a.slice(d))),b.replaceWith(h)})}function R(){const n=s.value.value.trim().toLowerCase();let t=0;g.value.querySelectorAll("h2.dom").forEach(e=>e.classList.toggle("hidden",!!n)),w.forEach(e=>{Q(e.el);const i=!n||e.text.includes(n);e.el.classList.toggle("hidden",!i),i&&(t++,n&&W(e.el,n))}),x.value=t===0,u.forEach(e=>{const i=document.getElementById(e.getAttribute("href").slice(1));e.classList.toggle("hidden",!!(i&&i.classList.contains("hidden")))})}function z(){clearTimeout(N),N=setTimeout(R,120)}function D(n){n.key==="/"&&document.activeElement!==s.value&&(n.preventDefault(),s.value.focus()),n.key==="Escape"&&document.activeElement===s.value&&(s.value.value="",R(),s.value.blur())}function S(){var e;const n=decodeURIComponent(location.hash.slice(1)),t=n?document.getElementById(n):null;k==null||k.classList.remove("flash"),k=null,!(!t||!((e=g.value)!=null&&e.contains(t)))&&(t.scrollIntoView({block:"start"}),t.offsetWidth,t.classList.add("flash"),k=t)}function E(){if(!P||!g.value)return;const n=P();let t=0,e=0,i=0,o=0;w.forEach(({el:b})=>{const h=n.get(b.id);if(!h)return;o++,t+=h.n,e+=h.seen,i+=h.wrong;const a=b.querySelector("h3");if(!a)return;const A=a.querySelector(".anch");if(!a.querySelector(".ql")){const r=document.createElement("a");r.className="ql",r.href="/interview/practice?note="+encodeURIComponent(b.id),r.target="_blank",r.rel="noopener",r.title=`Luyện ${h.n} câu hỏi thuộc mục này (mở tab mới)`,r.textContent=`ôn ${h.n} câu`,a.insertBefore(r,A)}let d=a.querySelector(".prg");d||(d=document.createElement("span"),d.className="prg",a.insertBefore(d,A)),d.textContent=`đã học ${h.seen}/${h.n}`+(h.wrong?` · ${h.wrong} sai`:""),d.className="prg"+(h.seen>=h.n?" full":"")+(h.wrong?" bad":"")}),M.value=o,p.value=t,L.value=e,q.value=i}function B(){T.reload(),E()}const O=()=>{document.hidden||B()},G=n=>{(!n.key||n.key.startsWith("interview."))&&B()};function J(n){var i,o;const t=(o=(i=n.target).closest)==null?void 0:o.call(i,"a");if(!t)return;const e=t.getAttribute("href")||"";e.startsWith("/")&&t.target!=="_blank"&&(n.metaKey||n.ctrlKey||n.shiftKey||n.altKey||n.button!==0||(n.preventDefault(),F.push(e)))}return $(()=>{w=[...g.value.querySelectorAll("section")].map(t=>({el:t,text:(t.textContent||"").toLowerCase()})),U(),Z(async()=>{const{PACKS:t,statsByNote:e}=await import("./packs-DDixvMNd.js");return{PACKS:t,statsByNote:e}},__vite__mapDeps([0,1,2])).then(({PACKS:t,statsByNote:e})=>{g.value&&(P=()=>e(t.interview,T.seen,T.wrong),E())});const n={};u.forEach(t=>n[t.getAttribute("href").slice(1)]=t),typeof IntersectionObserver<"u"&&(m=new IntersectionObserver(t=>{t.forEach(e=>{const i=n[e.target.id];!i||!e.isIntersecting||(u.forEach(o=>o.classList.remove("on")),i.classList.add("on"),i.scrollIntoView({block:"nearest"}))})},{rootMargin:"0px 0px -75% 0px"}),w.forEach(t=>m.observe(t.el))),location.hash&&S(),document.addEventListener("keydown",D),window.addEventListener("hashchange",S),window.addEventListener("storage",G),document.addEventListener("visibilitychange",O)}),tt(()=>{document.removeEventListener("keydown",D),window.removeEventListener("hashchange",S),window.removeEventListener("storage",G),document.removeEventListener("visibilitychange",O),m==null||m.disconnect()}),(n,t)=>{const e=it("router-link");return f(),v("div",at,[c("div",dt,[c("aside",null,[t[2]||(t[2]=c("h1",null,"Phỏng vấn Backend + AI",-1)),t[3]||(t[3]=c("div",{class:"sub"},"91 mục — 13 phase, AI cài vào từng phase",-1)),c("div",lt,[c("input",{ref_key:"qEl",ref:s,type:"search",placeholder:"Tìm trong tài liệu… (/)",autocomplete:"off",onInput:z},null,544)]),p.value?(f(),v("div",gt,[t[1]||(t[1]=C(" Đã học ",-1)),c("b",null,y(L.value)+"/"+y(p.value),1),C(" câu ("+y(Math.round(L.value/p.value*100))+"%) ",1),c("div",st,[c("i",{style:nt({width:L.value/p.value*100+"%"})},null,4)]),q.value?(f(),v("span",bt,y(q.value)+" câu đang sai",1)):(f(),v("span",pt,y(M.value)+" mục có câu hỏi",1))])):H("",!0),c("nav",{ref_key:"tocEl",ref:_,class:"toc"},null,512)]),c("main",null,[c("div",ut,[I(e,{to:"/interview/practice"},{default:V(()=>[...t[4]||(t[4]=[C("← Luyện phỏng vấn",-1)])]),_:1}),t[6]||(t[6]=c("span",{class:"spacer"},null,-1)),I(ct),I(e,{to:"/",style:{"text-decoration":"none"}},{default:V(()=>[...t[5]||(t[5]=[c("button",null,"🏠",-1)])]),_:1})]),t[0]||(j(-1,!0),(t[0]=c("div",{ref_key:"contentEl",ref:g,class:"content",innerHTML:et(rt),onClick:J},null,8,["innerHTML"])).cacheIndex=0,j(1),t[0]),x.value?(f(),v("div",mt,"Không tìm thấy mục nào khớp.")):H("",!0)])])])}}});export{Lt as default};
