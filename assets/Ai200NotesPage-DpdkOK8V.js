import{_ as V}from"./ThemeToggle.vue_vue_type_script_setup_true_lang-B9Rc_y3v.js";import{d as G,z as U,O as H,c as Q,a as r,i as v,b as k,w as I,$ as B,e as $,r as i,l as f,s as W,W as g,k as j,o as Y}from"./index-DeqZGsrA.js";import{u as X}from"./useStudyClock-B0smiK9L.js";const J=`
...
<h3 id="01-container/5c-acr-nang-cao">1.4 Geo-replication, Tasks, private link, dọn kho</h3>
...
<h3 id="01-container/trien-khai-acr">1.5 Triển khai ACR trên Azure — từng bước</h3>
...
<h3 id="01-container/trien-khai-appservice">2.3 Triển khai App Service (container) trên Azure — từng bước</h3>
...
<h3 id="01-container/trien-khai-aca">3.10 Triển khai Container Apps trên Azure — từng bước</h3>
...
<h3 id="01-container/trien-khai-aks">4.4 Triển khai AKS trên Azure — từng bước</h3>
...
<h3 id="02-data-services/d31-indexing-policy">1.4 Indexing policy</h3>
...
<h3 id="02-data-services/d32-consistency">1.5 Năm mức consistency — chọn theo câu chuyện của đề</h3>
...
<h3 id="02-data-services/a3-vector-search-trong-cosmos-db-for-nosql">1.6 Vector search trong Cosmos DB for NoSQL</h3>
...
<h3 id="02-data-services/a3b-full-text-va-hybrid-search">1.7 Full-text search và hybrid search (RRF)</h3>
...
<h3 id="02-data-services/a4-change-feed">1.8 Change feed</h3>
...
<h3 id="02-data-services/d7-duoi-nap-capo-ru-partition-change-feed">1.9 Dưới nắp capo: RU, partition split và change feed</h3>
...
<h3 id="02-data-services/trien-khai-cosmos">1.10 Triển khai Cosmos DB for NoSQL trên Azure — từng bước</h3>
...
<h3 id="02-data-services/b2-pgvector-thiet-lap">2.3 pgvector — thiết lập</h3>
...
<h3 id="02-data-services/b3-index-cho-vector-giam-latency-compute-overhead">2.4 Index cho vector — giảm latency &amp; compute overhead</h3>
...
<h3 id="02-data-services/d4-pgvector-chuyen-sau">2.5 pgvector chuyên sâu — HNSW vs IVFFlat và chi phí</h3>
...
<h3 id="02-data-services/d42-duoi-nap-capo-hnsw">2.6 Dưới nắp capo: HNSW duyệt đồ thị thế nào</h3>
...
<h3 id="02-data-services/b4-cau-hinh-compute-memory-storage-cho-vector-workload">2.7 Cấu hình compute / memory / storage cho vector workload</h3>
...
<h3 id="02-data-services/d41-sizing-postgres-cho-vector">2.8 Sizing PostgreSQL cho vector workload</h3>
...
<h3 id="02-data-services/b5-rag-voi-metadata-filter-nguyen-van-trong-outline">2.9 RAG với metadata filter (nguyên văn trong outline)</h3>
...
<h3 id="02-data-services/trien-khai-postgres">2.10 Triển khai PostgreSQL Flexible Server trên Azure — từng bước</h3>
...
<h3 id="02-data-services/trien-khai-redis">3.5 Triển khai Azure Managed Redis — từng bước</h3>
...
<h3 id="03-messaging-functions/trien-khai-servicebus">2.7 Triển khai Service Bus trên Azure — từng bước</h3>
...
<h3 id="03-messaging-functions/trien-khai-eventgrid">3.5 Triển khai Event Grid trên Azure — từng bước</h3>
...
<h3 id="03-messaging-functions/44-bao-mat-ket-noi">4.6 Bảo mật &amp; kết nối</h3>
...
<h3 id="03-messaging-functions/46-durable-functions-thuong-xuat-hien-trong-kich-ban-ai-pipe">4.7 Durable Functions (thường xuất hiện trong kịch bản AI pipeline dài)</h3>
...
<h3 id="03-messaging-functions/45-deploy">4.8 Deploy code</h3>
...
<h3 id="03-messaging-functions/trien-khai-functions">4.9 Triển khai Function App trên Azure — tạo hạ tầng từ đầu</h3>
...
<h3 id="04-security-monitoring/trien-khai-keyvault">1.7 Triển khai Key Vault trên Azure — từng bước</h3>
...
<h3 id="04-security-monitoring/trien-khai-appconfig">2.4 Triển khai App Configuration trên Azure — từng bước</h3>
...
<h3 id="04-security-monitoring/trien-khai-appinsights">3.6 Triển khai Application Insights + OpenTelemetry trên Azure</h3>
...
<hr>
<h2 id="04-security-monitoring/4-kql-kusto-query-language">4. KQL &amp; Log Analytics — truy vấn và cảnh báo</h2>
...
<h3 id="04-security-monitoring/49-bo-truy-van-tinh-huong-cookbook">4.3 Bộ truy vấn tình huống — cookbook</h3>
...
<h3 id="04-security-monitoring/43-alert-chi-phi">4.4 Alert &amp; chi phí</h3>
...
<h3 id="04-security-monitoring/trien-khai-monitor">4.5 Triển khai giám sát: diagnostic settings và alert rule</h3>
...
`,Z={class:"searchwrap"},tt={class:"wrap"},et={class:"progressbar"},h="ai200.tasks.v1",ct=G({__name:"Ai200NotesPage",setup(nt){X("ai200","notes");const d=i(),l=i(),a=i(),s=i(),A=i(),u=i(),p=i(),C=i(),S=i(),z=i();let c=[],m="",E=[];const T=e=>[...e.querySelectorAll("h2, h3")].map(t=>({lvl:+t.tagName[1],txt:t.textContent||"",id:t.id}));function N(){A.value.innerHTML=c.map(e=>{const t=e.id===m,o=e.dataset.weight?`<span class="w">${g(e.dataset.weight)}</span>`:"";return`<a class="navitem${t?" active":""}" href="#${e.id}"><span>${g(e.dataset.title)}</span>${o}</a>`+(t?'<div class="subnav">'+T(e).filter(n=>n.lvl===2).map(n=>`<a href="#${n.id}">${g(n.txt)}</a>`).join("")+"</div>":"")}).join("")}function q(e){u.value.innerHTML=T(e).map(t=>`<a class="${t.lvl===3?"h3":"h2"}" href="#${t.id}">${g(t.txt)}</a>`).join("")}function b(e,t){const o=c.find(n=>n.id===e)||c[0];if(m=o.id,a.value.hidden=!0,l.value.hidden=!1,c.forEach(n=>n.hidden=n!==o),N(),q(o),p.value.classList.remove("open"),t){const n=document.getElementById(t);if(n){n.scrollIntoView({block:"start"});return}}window.scrollTo(0,0)}function x(){const e=decodeURIComponent(location.hash.slice(1));if(!e)return b(c[0].id,null);b(e.split("/")[0],e.includes("/")?e:null)}function L(){const e=new Set(f(h,[]));d.value.querySelectorAll("li.task").forEach(t=>{const o=t.querySelector("input");o.checked=e.has(t.dataset.key),t.classList.toggle("done",o.checked),o.onchange=()=>{const n=new Set(f(h,[]));o.checked?n.add(t.dataset.key):n.delete(t.dataset.key),W(h,[...n]),t.classList.toggle("done",o.checked),D()}})}function D(){const e=d.value.querySelectorAll("li.task").length,t=f(h,[]).length;S.value.textContent=t+" / "+e+" mục đã xong",C.value.style.width=(e?t/e*100:0)+"%"}function P(){d.value.querySelectorAll(".copybtn").forEach(e=>{e.onclick=t=>{t.stopPropagation(),navigator.clipboard.writeText(e.parentElement.querySelector("code").innerText),e.textContent="đã copy",setTimeout(()=>e.textContent="copy",1200)}})}const _=[];function w(){c.forEach(e=>{e.querySelectorAll("h1,h2,h3,h4,p,li,td,th,pre").forEach(t=>{if(t.querySelector("p,ul,ol,pre,table,li"))return;const o=(t.textContent||"").replace(/\s+/g," ").trim();o.length>3&&_.push({docId:e.id,title:e.dataset.title||"",text:o})})})}function M(){const e=s.value.value.trim().toLowerCase();if(e.length<2){a.value.hidden=!0,l.value.hidden=!1;return}const t=[];for(const n of _)if(n.text.toLowerCase().includes(e)&&(t.push(n),t.length>=60))break;const o=new RegExp("("+e.replace(/[.*+?^${}()|[\]\\]/g,"\\$&")+")","ig");a.value.innerHTML=`<p style="color:var(--muted);font-size:13px">${t.length} kết quả cho “${g(s.value.value)}”</p>`+t.map(n=>`<div class="res" data-id="${n.docId}"><div class="doc">${g(n.title)}</div><div class="line">${g(n.text.slice(0,220)).replace(o,"<mark>$1</mark>")}</div></div>`).join(""),a.value.hidden=!1,l.value.hidden=!0,a.value.querySelectorAll(".res").forEach(n=>{n.onclick=()=>{s.value.value="",a.value.hidden=!0,l.value.hidden=!1,location.hash.slice(1)===n.dataset.id?b(n.dataset.id,null):location.hash=n.dataset.id}})}function O(e){e.key==="/"&&document.activeElement!==s.value&&(e.preventDefault(),s.value.focus()),e.key==="Escape"&&(s.value.value="",a.value.hidden=!0,l.value.hidden=!1,s.value.blur())}function K(){const e=c.filter(t=>t.hidden);e.forEach(t=>t.hidden=!1),window.print(),setTimeout(()=>e.forEach(t=>t.hidden=!0),400)}let y=!1;function F(){y||(y=!0,requestAnimationFrame(()=>{const e=[...d.value.querySelectorAll("section.doc:not([hidden]) h2, section.doc:not([hidden]) h3")];let t=null;for(const o of e)o.getBoundingClientRect().top<120&&(t=o);u.value.querySelectorAll("a").forEach(o=>o.classList.toggle("cur",!!t&&o.getAttribute("href")==="#"+t.id)),y=!1}))}function R(){d.value.style.setProperty("--topbar-h",z.value.offsetHeight+"px")}return U(()=>{c=[...d.value.querySelectorAll("section.doc")],m=c[0].id,w(),L(),P(),D(),x(),R();const e=[[window,"hashchange",x],[window,"resize",R],[window,"scroll",F],[document,"keydown",O]];e.forEach(([t,o,n])=>t.addEventListener(o,n)),E=e.map(([t,o,n])=>()=>t.removeEventListener(o,n))}),H(()=>E.forEach(e=>e())),(e,t)=>{const o=j("router-link");return Y(),Q("div",{ref_key:"rootEl",ref:d,class:"notes-ai"},[r("div",{ref_key:"topbarEl",ref:z,class:"topbar"},[r("button",{class:"btn menu-btn",onClick:t[0]||(t[0]=n=>p.value.classList.toggle("open"))},"☰"),t[4]||(t[4]=r("div",{class:"brand"},[r("span",{class:"dot"}),v("AI-200 "),r("small",null,"Azure AI Cloud Developer Associate · Tiếng Việt")],-1)),t[5]||(t[5]=r("div",{class:"spacer"},null,-1)),r("div",Z,[r("input",{ref_key:"searchEl",ref:s,placeholder:"Tìm trong toàn bộ tài liệu… (/)",autocomplete:"off",onInput:M},null,544)]),k(o,{class:"btn",to:"/ai-200/practice",title:"Mở trang luyện trắc nghiệm"},{default:I(()=>[...t[2]||(t[2]=[v("✎ Trắc nghiệm",-1)])]),_:1}),k(V),r("button",{class:"btn",title:"In / xuất PDF",onClick:K},"⎙"),k(o,{class:"btn",to:"/"},{default:I(()=>[...t[3]||(t[3]=[v("🏠",-1)])]),_:1})],512),r("div",tt,[r("aside",{ref_key:"sideEl",ref:p},[t[6]||(t[6]=r("div",{class:"navtitle"},"Tiến độ học",-1)),r("div",{ref_key:"pcountEl",ref:S,class:"pcount"},"0 / 0 mục đã xong",512),r("div",et,[r("div",{ref_key:"pbarEl",ref:C},null,512)]),t[7]||(t[7]=r("div",{class:"navtitle"},"Nội dung",-1)),r("nav",{ref_key:"navEl",ref:A},null,512)],512),r("main",null,[r("div",{ref_key:"resultsEl",ref:a,class:"results",hidden:""},null,512),t[1]||(B(-1,!0),(t[1]=r("article",{ref_key:"articleEl",ref:l,innerHTML:$(J)},null,8,["innerHTML"])).cacheIndex=1,B(1),t[1])]),r("div",{ref_key:"tocEl",ref:u,class:"toc"},null,512)])],512)}}});export{ct as default};
