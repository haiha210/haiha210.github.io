import{d as rt,j as p,s as at,x as ct,c as o,a,t as l,b as A,F as N,E as z,f,g as _,w as O,O as F,A as Y,r as it,m as ht,o as s,H as W,h as j}from"./index-DhgT_lph.js";import{_ as ot}from"./ThemeToggle.vue_vue_type_script_setup_true_lang-DqlLEWyz.js";import{u as st}from"./useStudyClock-jVxqQxbz.js";const P=`<h2 class="dom" id="d1">Phạm vi 1 · Nền tảng dữ liệu &amp; di trú</h2>
...
<tr><td><b>Azure Migrate</b></td><td>Khám phá và <b>đánh giá</b> cả data estate</td><td>Bất kỳ → khuyến nghị đích</td><td>Không áp dụng</td><td>—</td><td>Cho khuyến nghị đích, kích cỡ và ước tính chi phí hằng tháng; nay gánh luôn phần đánh giá tương thích của DMA</td></tr>
<tr><td><b>Azure SQL migration extension</b><br>(Azure Data Studio / VS Code)</td><td>Giao diện điều khiển DMS bản hiện hành</td><td>SQL Server → SQL DB / MI / VM</td><td>Theo chế độ DMS chọn</td><td>Có</td><td>Đây là cách chạy DMS hiện nay — đề nhắc "assessment rồi migrate trong Azure Data Studio" là nói tới nó</td></tr>
<tr><td><b>DMS</b> — Database Migration Service</td><td><b>Thực hiện</b> di trú, có quản lý</td><td>SQL Server → SQL DB / MI / VM; MySQL → Flexible</td><td>Có cả <b>offline</b> và <b>online</b></td><td>Có</td><td>DMS <b>không tự backup</b>: bạn cấp full + log backup, và nó <b>không dùng differential</b>. Bản classic Standard–Premium đã deprecate</td></tr>
<tr><td><b>DMA</b> — Data Migration Assistant</td><td>Đánh giá tương thích + di trú DB nhỏ</td><td>SQL Server → SQL DB / MI</td><td>Offline</td><td>Có</td><td><b>Khai tử 16/07/2025</b>. Đề còn nhắc DMA là đang hỏi vai trò <b>đánh giá</b>, không phải công cụ thực hiện</td></tr>
<tr><td><b>SSMA</b> — SQL Server Migration Assistant</td><td>Convert schema từ engine khác</td><td>Oracle, DB2, MySQL, Access, SAP ASE → SQL</td><td>Offline, dài</td><td>Có (convert)</td><td>Hai bước tách rời: convert object trước, chuyển dữ liệu sau</td></tr>
<tr><td><b>DEA</b> — Database Experimentation Assistant</td><td>So workload nguồn và đích <b>trước</b> khi chuyển</td><td>SQL Server → đích dự kiến</td><td>Không áp dụng</td><td>—</td><td>Bắt lại workload thật rồi phát lại, tìm truy vấn thoái lui hiệu năng</td></tr>
<tr><td><b>MI link</b></td><td>Di trú gần như không downtime, kiêm DR</td><td>SQL Server 2016+ → Managed Instance</td><td><b>Gần bằng 0</b></td><td>Có</td><td>Dựa trên distributed AG; trước cutover còn dùng làm bản đọc, và hỗ trợ failback về on-prem</td></tr>
<tr><td><b>Log Replay Service</b> (LRS)</td><td>Restore dần log vào MI, tự vận hành</td><td>SQL Server → Managed Instance</td><td>Ngắn</td><td>Có</td><td>Dùng khi không dùng được DMS; bạn tự đẩy full + log backup lên Azure Storage</td></tr>
<tr><td><b>Native backup/restore</b></td><td>Restore file backup từ URL</td><td>SQL Server → MI hoặc VM</td><td>Bằng thời gian restore</td><td>Có</td><td><b>Azure SQL Database KHÔNG nhận file <code>.bak</code></b> — chỉ MI và VM restore được</td></tr>
<tr><td><b>BACPAC</b> (data-tier application)</td><td>Xuất/nhập schema + dữ liệu một lần</td><td>SQL Server ↔ SQL DB / MI</td><td>Dài, theo kích thước DB</td><td>Có</td><td>Khác wizard <b>Import/Export Data</b> — wizard đó chỉ chép <b>dữ liệu</b>, không view/procedure/function</td></tr>
<tr><td><b>Transactional replication</b></td><td>Đồng bộ liên tục tới lúc cutover</td><td>SQL Server → SQL DB / MI</td><td>Rất ngắn</td><td>Một phần</td><td>Mọi bảng publish phải có <b>primary key</b>; hợp khi chỉ chuyển một phần bảng</td></tr>
<tr><td><b>ADF</b> — Azure Data Factory</td><td>Pipeline <b>ETL/ELT</b>, chép và biến đổi dữ liệu</td><td>90+ nguồn → Synapse, Fabric, Data Lake, Azure SQL</td><td>Không áp dụng</td><td><b>KHÔNG</b></td><td>Chạy lặp lại theo lịch, nạp tăng dần; có Azure-SSIS IR để lift package SSIS cũ. Không thay được di trú database</td></tr>
<tr><td><b>SQL Data Sync</b></td><td>Giữ nhiều database <b>đồng bộ lâu dài</b></td><td>SQL DB ↔ SQL Server / SQL DB</td><td>Không áp dụng</td><td>Không</td><td>Hub phải là Azure SQL Database; bảng cần primary key; <b>KHÔNG dùng với MI</b></td></tr>
<tr><td><b>MI database copy / move</b></td><td>Chuyển database giữa hai MI</td><td>MI → MI</td><td>Ngắn</td><td>Có</td><td>Không mất dữ liệu; <i>copy</i> thì bản gốc vẫn online rồi hai bên ngừng đồng bộ; không xong trong <b>24 giờ</b> là bị huỷ</td></tr>
</table></div>
