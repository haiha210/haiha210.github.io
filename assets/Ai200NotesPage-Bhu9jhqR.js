import{_ as F}from"./ThemeToggle.vue_vue_type_script_setup_true_lang-Ca5tM9l2.js";import{d as V,z as G,O as H,c as Q,a as i,i as l,b as u,w as f,$ as B,e as W,r,l as A,s as j,W as h,k as X,o as $}from"./index-BvUv8dKS.js";import{u as Y}from"./useStudyClock-y-4x9X8P.js";const J=`
...
<h2 id="02-data-services/e-checklist-on-domain-2">17. Checklist ôn Domain 2</h2>
...
<h2 id="04-security-monitoring/6-checklist-on-domain-4">12. Checklist ôn Domain 4</h2>
...
`,Z={class:"searchwrap"},tt={class:"wrap"},nt={class:"progressbar"},p="ai200.tasks.v1",ct=V({__name:"Ai200NotesPage",setup(et){Y("ai200","notes");const s=r(),g=r(),a=r(),d=r(),C=r(),m=r(),b=r(),S=r(),T=r(),E=r();let c=[],y="",x=[];const D=n=>[...n.querySelectorAll("h2, h3")].map(t=>({lvl:+t.tagName[1],txt:t.textContent||"",id:t.id}));function P(){C.value.innerHTML=c.map(n=>{const t=n.id===y,o=n.dataset.weight?`<span class="w">${h(n.dataset.weight)}</span>`:"";return`<a class="navitem${t?" active":""}" href="#${n.id}"><span>${h(n.dataset.title)}</span>${o}</a>`+(t?'<div class="subnav">'+D(n).filter(e=>e.lvl===2).map(e=>`<a href="#${e.id}">${h(e.txt)}</a>`).join("")+"</div>":"")}).join("")}function L(n){m.value.innerHTML=D(n).map(t=>`<a class="${t.lvl===3?"h3":"h2"}" href="#${t.id}">${h(t.txt)}</a>`).join("")}function v(n,t){const o=c.find(e=>e.id===n)||c[0];if(y=o.id,a.value.hidden=!0,g.value.hidden=!1,c.forEach(e=>e.hidden=e!==o),P(),L(o),b.value.classList.remove("open"),t){const e=document.getElementById(t);if(e){e.scrollIntoView({block:"start"});return}}window.scrollTo(0,0)}function R(){const n=decodeURIComponent(location.hash.slice(1));if(!n)return v(c[0].id,null);v(n.split("/")[0],n.includes("/")?n:null)}function q(){const n=new Set(A(p,[]));s.value.querySelectorAll("li.task").forEach(t=>{const o=t.querySelector("input");o.checked=n.has(t.dataset.key),t.classList.toggle("done",o.checked),o.onchange=()=>{const e=new Set(A(p,[]));o.checked?e.add(t.dataset.key):e.delete(t.dataset.key),j(p,[...e]),t.classList.toggle("done",o.checked),_()}})}function _(){const n=s.value.querySelectorAll("li.task").length,t=A(p,[]).length;T.value.textContent=t+" / "+n+" mục đã xong",S.value.style.width=(n?t/n*100:0)+"%"}function I(){s.value.querySelectorAll(".copybtn").forEach(n=>{n.onclick=t=>{t.stopPropagation(),navigator.clipboard.writeText(n.parentElement.querySelector("code").innerText),n.textContent="đã copy",setTimeout(()=>n.textContent="copy",1200)}})}const N=[];function M(){c.forEach(n=>{n.querySelectorAll("h1,h2,h3,h4,p,li,td,th,pre").forEach(t=>{if(t.querySelector("p,ul,ol,pre,table,li"))return;const o=(t.textContent||"").replace(/\s+/g," ").trim();o.length>3&&N.push({docId:n.id,title:n.dataset.title||"",text:o})})})}function w(){const n=d.value.value.trim().toLowerCase();if(n.length<2){a.value.hidden=!0,g.value.hidden=!1;return}const t=[];for(const e of N)if(e.text.toLowerCase().includes(n)&&(t.push(e),t.length>=60))break;const o=new RegExp("("+n.replace(/[.*+?^${}()|[\]\\]/g,"\\$&")+")","ig");a.value.innerHTML=`<p style="color:var(--muted);font-size:13px">${t.length} kết quả cho “${h(d.value.value)}”</p>`+t.map(e=>`<div class="res" data-id="${e.docId}"><div class="doc">${h(e.title)}</div><div class="line">${h(e.text.slice(0,220)).replace(o,"<mark>$1</mark>")}</div></div>`).join(""),a.value.hidden=!1,g.value.hidden=!0,a.value.querySelectorAll(".res").forEach(e=>{e.onclick=()=>{d.value.value="",a.value.hidden=!0,g.value.hidden=!1,location.hash.slice(1)===e.dataset.id?v(e.dataset.id,null):location.hash=e.dataset.id}})}function K(n){n.key==="/"&&document.activeElement!==d.value&&(n.preventDefault(),d.value.focus()),n.key==="Escape"&&(d.value.value="",a.value.hidden=!0,g.value.hidden=!1,d.value.blur())}function O(){const n=c.filter(t=>t.hidden);n.forEach(t=>t.hidden=!1),window.print(),setTimeout(()=>n.forEach(t=>t.hidden=!0),400)}let k=!1;function U(){k||(k=!0,requestAnimationFrame(()=>{const n=[...s.value.querySelectorAll("section.doc:not([hidden]) h2, section.doc:not([hidden]) h3")];let t=null;for(const o of n)o.getBoundingClientRect().top<120&&(t=o);m.value.querySelectorAll("a").forEach(o=>o.classList.toggle("cur",!!t&&o.getAttribute("href")==="#"+t.id)),k=!1}))}function z(){s.value.style.setProperty("--topbar-h",E.value.offsetHeight+"px")}return G(()=>{c=[...s.value.querySelectorAll("section.doc")],y=c[0].id,M(),q(),I(),_(),R(),z();const n=[[window,"hashchange",R],[window,"resize",z],[window,"scroll",U],[document,"keydown",K]];n.forEach(([t,o,e])=>t.addEventListener(o,e)),x=n.map(([t,o,e])=>()=>t.removeEventListener(o,e))}),H(()=>x.forEach(n=>n())),(n,t)=>{const o=X("router-link");return $(),Q("div",{ref_key:"rootEl",ref:s,class:"notes-ai"},[i("div",{ref_key:"topbarEl",ref:E,class:"topbar"},[i("button",{class:"btn menu-btn",onClick:t[0]||(t[0]=e=>b.value.classList.toggle("open"))},"☰"),t[5]||(t[5]=i("div",{class:"brand"},[i("span",{class:"dot"}),l("AI-200 "),i("small",null,"Azure AI Cloud Developer Associate · Tiếng Việt")],-1)),t[6]||(t[6]=i("div",{class:"spacer"},null,-1)),i("div",Z,[i("input",{ref_key:"searchEl",ref:d,placeholder:"Tìm trong toàn bộ tài liệu… (/)",autocomplete:"off",onInput:w},null,544)]),u(o,{class:"btn",to:"/ai-200/practice",title:"Mở trang luyện trắc nghiệm"},{default:f(()=>[...t[2]||(t[2]=[l("✎ Trắc nghiệm",-1)])]),_:1}),u(o,{class:"btn",to:"/ai-200/labs",title:"Mở trang lab thực hành"},{default:f(()=>[...t[3]||(t[3]=[l("🧪 Thực hành",-1)])]),_:1}),u(F),i("button",{class:"btn",title:"In / xuất PDF",onClick:O},"⎙"),u(o,{class:"btn",to:"/"},{default:f(()=>[...t[4]||(t[4]=[l("🏠",-1)])]),_:1})],512),i("div",tt,[i("aside",{ref_key:"sideEl",ref:b},[t[7]||(t[7]=i("div",{class:"navtitle"},"Tiến độ học",-1)),i("div",{ref_key:"pcountEl",ref:T,class:"pcount"},"0 / 0 mục đã xong",512),i("div",nt,[i("div",{ref_key:"pbarEl",ref:S},null,512)]),t[8]||(t[8]=i("div",{class:"navtitle"},"Nội dung",-1)),i("nav",{ref_key:"navEl",ref:C},null,512)],512),i("main",null,[i("div",{ref_key:"resultsEl",ref:a,class:"results",hidden:""},null,512),t[1]||(B(-1,!0),(t[1]=i("article",{ref_key:"articleEl",ref:g,innerHTML:W(J)},null,8,["innerHTML"])).cacheIndex=1,B(1),t[1])]),i("div",{ref_key:"tocEl",ref:m,class:"toc"},null,512)])],512)}}});export{ct as default};
