import{_ as U}from"./ThemeToggle.vue_vue_type_script_setup_true_lang-C2fHyvi2.js";import{d as V,z as G,O as H,c as Q,a as i,i as l,b as u,w as f,$ as L,e as W,r as c,l as A,s as j,W as h,k as X,o as Y}from"./index-BX14n8MT.js";import{u as $}from"./useStudyClock-D8NiLTQg.js";const J=`
...
`,Z={class:"searchwrap"},tt={class:"wrap"},nt={class:"progressbar"},p="ai200.tasks.v1",rt=V({__name:"Ai200NotesPage",setup(et){$("ai200","notes");const s=c(),g=c(),a=c(),d=c(),C=c(),m=c(),b=c(),S=c(),T=c(),E=c();let r=[],y="",x=[];const _=n=>[...n.querySelectorAll("h2, h3")].map(t=>({lvl:+t.tagName[1],txt:t.textContent||"",id:t.id}));function B(){C.value.innerHTML=r.map(n=>{const t=n.id===y,o=n.dataset.weight?`<span class="w">${h(n.dataset.weight)}</span>`:"";return`<a class="navitem${t?" active":""}" href="#${n.id}"><span>${h(n.dataset.title)}</span>${o}</a>`+(t?'<div class="subnav">'+_(n).filter(e=>e.lvl===2).map(e=>`<a href="#${e.id}">${h(e.txt)}</a>`).join("")+"</div>":"")}).join("")}function I(n){m.value.innerHTML=_(n).map(t=>`<a class="${t.lvl===3?"h3":"h2"}" href="#${t.id}">${h(t.txt)}</a>`).join("")}function v(n,t){const o=r.find(e=>e.id===n)||r[0];if(y=o.id,a.value.hidden=!0,g.value.hidden=!1,r.forEach(e=>e.hidden=e!==o),B(),I(o),b.value.classList.remove("open"),t){const e=document.getElementById(t);if(e){e.scrollIntoView({block:"start"});return}}window.scrollTo(0,0)}function D(){const n=decodeURIComponent(location.hash.slice(1));if(!n)return v(r[0].id,null);v(n.split("/")[0],n.includes("/")?n:null)}function z(){const n=new Set(A(p,[]));s.value.querySelectorAll("li.task").forEach(t=>{const o=t.querySelector("input");o.checked=n.has(t.dataset.key),t.classList.toggle("done",o.checked),o.onchange=()=>{const e=new Set(A(p,[]));o.checked?e.add(t.dataset.key):e.delete(t.dataset.key),j(p,[...e]),t.classList.toggle("done",o.checked),R()}})}function R(){const n=s.value.querySelectorAll("li.task").length,t=A(p,[]).length;T.value.textContent=t+" / "+n+" mục đã xong",S.value.style.width=(n?t/n*100:0)+"%"}function P(){s.value.querySelectorAll(".copybtn").forEach(n=>{n.onclick=t=>{t.stopPropagation(),navigator.clipboard.writeText(n.parentElement.querySelector("code").innerText),n.textContent="đã copy",setTimeout(()=>n.textContent="copy",1200)}})}const N=[];function M(){r.forEach(n=>{n.querySelectorAll("h1,h2,h3,h4,p,li,td,th,pre").forEach(t=>{if(t.querySelector("p,ul,ol,pre,table,li"))return;const o=(t.textContent||"").replace(/\s+/g," ").trim();o.length>3&&N.push({docId:n.id,title:n.dataset.title||"",text:o})})})}function w(){const n=d.value.value.trim().toLowerCase();if(n.length<2){a.value.hidden=!0,g.value.hidden=!1;return}const t=[];for(const e of N)if(e.text.toLowerCase().includes(n)&&(t.push(e),t.length>=60))break;const o=new RegExp("("+n.replace(/[.*+?^${}()|[\]\\]/g,"\\$&")+")","ig");a.value.innerHTML=`<p style="color:var(--muted);font-size:13px">${t.length} kết quả cho “${h(d.value.value)}”</p>`+t.map(e=>`<div class="res" data-id="${e.docId}"><div class="doc">${h(e.title)}</div><div class="line">${h(e.text.slice(0,220)).replace(o,"<mark>$1</mark>")}</div></div>`).join(""),a.value.hidden=!1,g.value.hidden=!0,a.value.querySelectorAll(".res").forEach(e=>{e.onclick=()=>{d.value.value="",a.value.hidden=!0,g.value.hidden=!1,location.hash.slice(1)===e.dataset.id?v(e.dataset.id,null):location.hash=e.dataset.id}})}function K(n){n.key==="/"&&document.activeElement!==d.value&&(n.preventDefault(),d.value.focus()),n.key==="Escape"&&(d.value.value="",a.value.hidden=!0,g.value.hidden=!1,d.value.blur())}function O(){const n=r.filter(t=>t.hidden);n.forEach(t=>t.hidden=!1),window.print(),setTimeout(()=>n.forEach(t=>t.hidden=!0),400)}let k=!1;function F(){k||(k=!0,requestAnimationFrame(()=>{const n=[...s.value.querySelectorAll("section.doc:not([hidden]) h2, section.doc:not([hidden]) h3")];let t=null;for(const o of n)o.getBoundingClientRect().top<120&&(t=o);m.value.querySelectorAll("a").forEach(o=>o.classList.toggle("cur",!!t&&o.getAttribute("href")==="#"+t.id)),k=!1}))}function q(){s.value.style.setProperty("--topbar-h",E.value.offsetHeight+"px")}return G(()=>{r=[...s.value.querySelectorAll("section.doc")],y=r[0].id,M(),z(),P(),R(),D(),q();const n=[[window,"hashchange",D],[window,"resize",q],[window,"scroll",F],[document,"keydown",K]];n.forEach(([t,o,e])=>t.addEventListener(o,e)),x=n.map(([t,o,e])=>()=>t.removeEventListener(o,e))}),H(()=>x.forEach(n=>n())),(n,t)=>{const o=X("router-link");return Y(),Q("div",{ref_key:"rootEl",ref:s,class:"notes-ai"},[i("div",{ref_key:"topbarEl",ref:E,class:"topbar"},[i("button",{class:"btn menu-btn",onClick:t[0]||(t[0]=e=>b.value.classList.toggle("open"))},"☰"),t[5]||(t[5]=i("div",{class:"brand"},[i("span",{class:"dot"}),l("AI-200 "),i("small",null,"Azure AI Cloud Developer Associate · Tiếng Việt")],-1)),t[6]||(t[6]=i("div",{class:"spacer"},null,-1)),i("div",Z,[i("input",{ref_key:"searchEl",ref:d,placeholder:"Tìm trong toàn bộ tài liệu… (/)",autocomplete:"off",onInput:w},null,544)]),u(o,{class:"btn",to:"/ai-200/practice",title:"Mở trang luyện trắc nghiệm"},{default:f(()=>[...t[2]||(t[2]=[l("✎ Trắc nghiệm",-1)])]),_:1}),u(o,{class:"btn",to:"/ai-200/labs",title:"Mở trang lab thực hành"},{default:f(()=>[...t[3]||(t[3]=[l("🧪 Thực hành",-1)])]),_:1}),u(U),i("button",{class:"btn",title:"In / xuất PDF",onClick:O},"⎙"),u(o,{class:"btn",to:"/"},{default:f(()=>[...t[4]||(t[4]=[l("🏠",-1)])]),_:1})],512),i("div",tt,[i("aside",{ref_key:"sideEl",ref:b},[t[7]||(t[7]=i("div",{class:"navtitle"},"Tiến độ học",-1)),i("div",{ref_key:"pcountEl",ref:T,class:"pcount"},"0 / 0 mục đã xong",512),i("div",nt,[i("div",{ref_key:"pbarEl",ref:S},null,512)]),t[8]||(t[8]=i("div",{class:"navtitle"},"Nội dung",-1)),i("nav",{ref_key:"navEl",ref:C},null,512)],512),i("main",null,[i("div",{ref_key:"resultsEl",ref:a,class:"results",hidden:""},null,512),t[1]||(L(-1,!0),(t[1]=i("article",{ref_key:"articleEl",ref:g,innerHTML:W(J)},null,8,["innerHTML"])).cacheIndex=1,L(1),t[1])]),i("div",{ref_key:"tocEl",ref:m,class:"toc"},null,512)])],512)}}});export{rt as default};
