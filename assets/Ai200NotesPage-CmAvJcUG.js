...
import{d as Q,z as W,O as j,c as X,a as i,i as u,b as p,w as C,$ as P,e as Y,r as c,l as S,s as $,W as g,a0 as J,k as Z,o as tt}from"./index-BUGOdOIA.js";import{_ as nt}from"./ThemeToggle.vue_vue_type_script_setup_true_lang-BtF8Nuk9.js";import{u as et}from"./useStudyClock-CHiT4IK7.js";const ot=`
...
<h3 id="02-data-services/d42-duoi-nap-capo-hnsw">7.2 Dưới nắp capo: HNSW hoạt động thế nào</h3>
...
<h3 id="02-data-services/d43-duoi-nap-capo-ivfflat">7.3 Dưới nắp capo: IVFFlat hoạt động thế nào</h3>
...
<h3 id="02-data-services/d44-chon-hnsw-hay-ivfflat">7.4 Chọn cái nào — và cái bẫy chung khi có bộ lọc</h3>
<table><thead><tr><th></th><th>HNSW</th><th>IVFFlat</th></tr></thead><tbody>
...
<li><em>Kiểm chứng: mọi con số và thông báo lỗi ở ba mục 7.2–7.4 đã chạy thật trên pgvector 0.8.6 trong lab — xem <code>labs/ai-200/labs/02-index-hnsw-ivfflat.sql</code> và <code>03-rag-filter-hybrid.sql</code>. Nguồn đối chiếu: <a href="https://github.com/pgvector/pgvector" target="_blank" rel="noopener">README của pgvector</a>, đọc ngày 19/09/2026.</em></li></ul>
<h3 id="02-data-services/d45-chon-toan-tu-nao">7.5 Chọn toán tử nào — <code>&lt;=&gt;</code>, <code>&lt;-&gt;</code> hay <code>&lt;#&gt;</code></h3>
...
<h2 id="03-messaging-functions/10-han-muc-auto-forward-tier">10. Hạn mức Service Bus, auto-forward và chọn tier</h2>
...
<h3 id="03-messaging-functions/10a-han-muc">10.1 Hạn mức phải nhớ</h3>
...
<h3 id="03-messaging-functions/10b-auto-forward">10.2 Auto-forward — nối chuỗi thực thể, và bốn ràng buộc hay bị bỏ qua</h3>
...
<h3 id="03-messaging-functions/10c-chon-tier">10.3 Chọn tier</h3>
...
<h2 id="03-messaging-functions/5-checklist-on-domain-3">11. Checklist ôn Domain 3</h2>
...
`,it={class:"searchwrap"},ct={class:"wrap"},rt={class:"progressbar"},m="ai200.tasks.v1",gt=Q({__name:"Ai200NotesPage",setup(dt){et("ai200","notes");const d=c(),l=c(),a=c(),s=c(),E=c(),b=c(),y=c(),x=c(),T=c(),_=c();let r=[],v="",R=[];const I=n=>{const t=n.cloneNode(!0);return t.querySelectorAll(".ql, .anch").forEach(o=>o.remove()),t.textContent||""},D=n=>[...n.querySelectorAll("h2, h3")].map(t=>({lvl:+t.tagName[1],txt:I(t),id:t.id}));function z(){E.value.innerHTML=r.map(n=>{const t=n.id===v,o=n.dataset.weight?`<span class="w">${g(n.dataset.weight)}</span>`:"";return`<a class="navitem${t?" active":""}" href="#${n.id}"><span>${g(n.dataset.title)}</span>${o}</a>`+(t?'<div class="subnav">'+D(n).filter(e=>e.lvl===2).map(e=>`<a href="#${e.id}">${g(e.txt)}</a>`).join("")+"</div>":"")}).join("")}function M(n){b.value.innerHTML=D(n).map(t=>`<a class="${t.lvl===3?"h3":"h2"}" href="#${t.id}">${g(t.txt)}</a>`).join("")}function k(n,t){const o=r.find(e=>e.id===n)||r[0];if(v=o.id,a.value.hidden=!0,l.value.hidden=!1,r.forEach(e=>e.hidden=e!==o),z(),M(o),y.value.classList.remove("open"),t){const e=document.getElementById(t);if(e){e.scrollIntoView({block:"start"});return}}window.scrollTo(0,0)}function q(){const n=decodeURIComponent(location.hash.slice(1));if(!n)return k(r[0].id,null);k(n.split("/")[0],n.includes("/")?n:null)}function w(){const n=new Set(S(m,[]));d.value.querySelectorAll("li.task").forEach(t=>{const o=t.querySelector("input");o.checked=n.has(t.dataset.key),t.classList.toggle("done",o.checked),o.onchange=()=>{const e=new Set(S(m,[]));o.checked?e.add(t.dataset.key):e.delete(t.dataset.key),$(m,[...e]),t.classList.toggle("done",o.checked),N()}})}function N(){const n=d.value.querySelectorAll("li.task").length,t=S(m,[]).length;T.value.textContent=t+" / "+n+" mục đã xong",x.value.style.width=(n?t/n*100:0)+"%"}function K(){d.value.querySelectorAll(".copybtn").forEach(n=>{n.onclick=t=>{t.stopPropagation(),navigator.clipboard.writeText(n.parentElement.querySelector("code").innerText),n.textContent="đã copy",setTimeout(()=>n.textContent="copy",1200)}})}const L=[];function O(){r.forEach(n=>{n.querySelectorAll("h1,h2,h3,h4,p,li,td,th,pre").forEach(t=>{if(t.querySelector("p,ul,ol,pre,table,li"))return;const o=(t.textContent||"").replace(/\s+/g," ").trim();o.length>3&&L.push({docId:n.id,title:n.dataset.title||"",text:o})})})}function F(){const n=s.value.value.trim().toLowerCase();if(n.length<2){a.value.hidden=!0,l.value.hidden=!1;return}const t=[];for(const e of L)if(e.text.toLowerCase().includes(n)&&(t.push(e),t.length>=60))break;const o=new RegExp("("+n.replace(/[.*+?^${}()|[\]\\]/g,"\\$&")+")","ig");a.value.innerHTML=`<p style="color:var(--muted);font-size:13px">${t.length} kết quả cho “${g(s.value.value)}”</p>`+t.map(e=>`<div class="res" data-id="${e.docId}"><div class="doc">${g(e.title)}</div><div class="line">${g(e.text.slice(0,220)).replace(o,"<mark>$1</mark>")}</div></div>`).join(""),a.value.hidden=!1,l.value.hidden=!0,a.value.querySelectorAll(".res").forEach(e=>{e.onclick=()=>{s.value.value="",a.value.hidden=!0,l.value.hidden=!1,location.hash.slice(1)===e.dataset.id?k(e.dataset.id,null):location.hash=e.dataset.id}})}function V(n){n.key==="/"&&document.activeElement!==s.value&&(n.preventDefault(),s.value.focus()),n.key==="Escape"&&(s.value.value="",a.value.hidden=!0,l.value.hidden=!1,s.value.blur())}function U(){const n=r.filter(t=>t.hidden);n.forEach(t=>t.hidden=!1),window.print(),setTimeout(()=>n.forEach(t=>t.hidden=!0),400)}let f=!1;function G(){f||(f=!0,requestAnimationFrame(()=>{const n=[...d.value.querySelectorAll("section.doc:not([hidden]) h2, section.doc:not([hidden]) h3")];let t=null;for(const o of n)o.getBoundingClientRect().top<120&&(t=o);b.value.querySelectorAll("a").forEach(o=>o.classList.toggle("cur",!!t&&o.getAttribute("href")==="#"+t.id)),f=!1}))}function H(){J(async()=>{const{PACKS:n,countByNote:t}=await import("./packs-B2WMnI0p.js");return{PACKS:n,countByNote:t}},__vite__mapDeps([0,1,2])).then(({PACKS:n,countByNote:t})=>{const o=t(n.ai200);d.value.querySelectorAll("section.doc h2[id], section.doc h3[id]").forEach(e=>{const A=o.get(e.id);if(!A)return;const h=document.createElement("a");h.className="ql",h.href="/ai-200/practice?doc="+encodeURIComponent(e.id),h.target="_blank",h.rel="noopener",h.title=`Luyện ${A} câu hỏi thuộc mục này (mở tab mới)`,h.textContent=`ôn ${A} câu`,e.insertBefore(h,e.querySelector(".anch"))})})}function B(){d.value.style.setProperty("--topbar-h",_.value.offsetHeight+"px")}return W(()=>{r=[...d.value.querySelectorAll("section.doc")],v=r[0].id,O(),H(),w(),K(),N(),q(),B();const n=[[window,"hashchange",q],[window,"resize",B],[window,"scroll",G],[document,"keydown",V]];n.forEach(([t,o,e])=>t.addEventListener(o,e)),R=n.map(([t,o,e])=>()=>t.removeEventListener(o,e))}),j(()=>R.forEach(n=>n())),(n,t)=>{const o=Z("router-link");return tt(),X("div",{ref_key:"rootEl",ref:d,class:"notes-ai"},[i("div",{ref_key:"topbarEl",ref:_,class:"topbar"},[i("button",{class:"btn menu-btn",onClick:t[0]||(t[0]=e=>y.value.classList.toggle("open"))},"☰"),t[5]||(t[5]=i("div",{class:"brand"},[i("span",{class:"dot"}),u("AI-200 "),i("small",null,"Azure AI Cloud Developer Associate · Tiếng Việt")],-1)),t[6]||(t[6]=i("div",{class:"spacer"},null,-1)),i("div",it,[i("input",{ref_key:"searchEl",ref:s,placeholder:"Tìm trong toàn bộ tài liệu… (/)",autocomplete:"off",onInput:F},null,544)]),p(o,{class:"btn",to:"/ai-200/practice",title:"Mở trang luyện trắc nghiệm"},{default:C(()=>[...t[2]||(t[2]=[u("✎ Trắc nghiệm",-1)])]),_:1}),p(o,{class:"btn",to:"/ai-200/labs",title:"Mở trang lab thực hành"},{default:C(()=>[...t[3]||(t[3]=[u("🧪 Thực hành",-1)])]),_:1}),p(nt),i("button",{class:"btn",title:"In / xuất PDF",onClick:U},"⎙"),p(o,{class:"btn",to:"/"},{default:C(()=>[...t[4]||(t[4]=[u("🏠",-1)])]),_:1})],512),i("div",ct,[i("aside",{ref_key:"sideEl",ref:y},[t[7]||(t[7]=i("div",{class:"navtitle"},"Tiến độ học",-1)),i("div",{ref_key:"pcountEl",ref:T,class:"pcount"},"0 / 0 mục đã xong",512),i("div",rt,[i("div",{ref_key:"pbarEl",ref:x},null,512)]),t[8]||(t[8]=i("div",{class:"navtitle"},"Nội dung",-1)),i("nav",{ref_key:"navEl",ref:E},null,512)],512),i("main",null,[i("div",{ref_key:"resultsEl",ref:a,class:"results",hidden:""},null,512),t[1]||(P(-1,!0),(t[1]=i("article",{ref_key:"articleEl",ref:l,innerHTML:Y(ot)},null,8,["innerHTML"])).cacheIndex=1,P(1),t[1])]),i("div",{ref_key:"tocEl",ref:b,class:"toc"},null,512)])],512)}}});export{gt as default};
