...
import{d as Q,z as W,O as $,c as j,a as i,i as u,b as p,w as S,$ as L,e as X,r,l as C,s as Y,W as g,a0 as J,k as Z,o as tt}from"./index-CgIC4Xuz.js";import{_ as nt}from"./ThemeToggle.vue_vue_type_script_setup_true_lang-BoFe6Pjv.js";import{u as et}from"./useStudyClock-DcRSRh2f.js";const ot=`
...
<h3 id="01-container/trien-khai-acr">1.8 Triển khai ACR trên Azure — từng bước</h3>
...
<h3 id="01-container/trien-khai-aca">3.12 Triển khai Container Apps trên Azure — từng bước</h3>
...
<h3 id="01-container/trien-khai-aks">4.15 Triển khai AKS trên Azure — từng bước</h3>
...
<h3 id="02-data-services/trien-khai-cosmos">1.12 Triển khai Cosmos DB for NoSQL trên Azure — từng bước</h3>
...
<tr><td>Kích thước index</td><td>lớn (cỡ ~2× dữ liệu vector)</td><td>nhỏ</td></tr>
...
<p><strong>Mặc định chọn HNSW.</strong> Chọn IVFFlat khi: dữ liệu nạp một lần rồi gần như không đổi, index chắc chắn không vừa RAM, hoặc thời gian build là ràng buộc thật (ví dụ phải dựng lại index trong cửa sổ bảo trì ngắn).</p>
...
<h3 id="02-data-services/trien-khai-postgres">2.23 Triển khai PostgreSQL Flexible Server trên Azure — từng bước</h3>
...
<h3 id="02-data-services/trien-khai-redis">3.6 Triển khai Azure Managed Redis — từng bước</h3>
...
<h3 id="03-messaging-functions/trien-khai-servicebus">2.11 Triển khai Service Bus trên Azure — từng bước</h3>
...
<h3 id="03-messaging-functions/trien-khai-eventgrid">3.6 Triển khai Event Grid trên Azure — từng bước</h3>
...
<h3 id="03-messaging-functions/trien-khai-functions">4.12 Triển khai Function App trên Azure — tạo hạ tầng từ đầu</h3>
...
<h3 id="04-security-monitoring/trien-khai-appinsights">3.7 Triển khai Application Insights + OpenTelemetry trên Azure</h3>
...
<h3 id="04-security-monitoring/trien-khai-monitor">5.6 Triển khai giám sát: diagnostic settings và alert rule</h3>
...
`,it={class:"searchwrap"},rt={class:"wrap"},ct={class:"progressbar"},m="ai200.tasks.v1",gt=Q({__name:"Ai200NotesPage",setup(at){et("ai200","notes");const a=r(),l=r(),s=r(),d=r(),E=r(),b=r(),y=r(),T=r(),x=r(),_=r();let c=[],v="",D=[];const P=n=>{const t=n.cloneNode(!0);return t.querySelectorAll(".ql, .anch").forEach(o=>o.remove()),t.textContent||""},R=n=>[...n.querySelectorAll("h2, h3")].map(t=>({lvl:+t.tagName[1],txt:P(t),id:t.id}));function B(){E.value.innerHTML=c.map(n=>{const t=n.id===v,o=n.dataset.weight?`<span class="w">${g(n.dataset.weight)}</span>`:"";return`<a class="navitem${t?" active":""}" href="#${n.id}"><span>${g(n.dataset.title)}</span>${o}</a>`+(t?'<div class="subnav">'+R(n).filter(e=>e.lvl===2).map(e=>`<a href="#${e.id}">${g(e.txt)}</a>`).join("")+"</div>":"")}).join("")}function M(n){b.value.innerHTML=R(n).map(t=>`<a class="${t.lvl===3?"h3":"h2"}" href="#${t.id}">${g(t.txt)}</a>`).join("")}function k(n,t){const o=c.find(e=>e.id===n)||c[0];if(v=o.id,s.value.hidden=!0,l.value.hidden=!1,c.forEach(e=>e.hidden=e!==o),B(),M(o),y.value.classList.remove("open"),t){const e=document.getElementById(t);if(e){e.scrollIntoView({block:"start"});return}}window.scrollTo(0,0)}function z(){const n=decodeURIComponent(location.hash.slice(1));if(!n)return k(c[0].id,null);k(n.split("/")[0],n.includes("/")?n:null)}function w(){const n=new Set(C(m,[]));a.value.querySelectorAll("li.task").forEach(t=>{const o=t.querySelector("input");o.checked=n.has(t.dataset.key),t.classList.toggle("done",o.checked),o.onchange=()=>{const e=new Set(C(m,[]));o.checked?e.add(t.dataset.key):e.delete(t.dataset.key),Y(m,[...e]),t.classList.toggle("done",o.checked),I()}})}function I(){const n=a.value.querySelectorAll("li.task").length,t=C(m,[]).length;x.value.textContent=t+" / "+n+" mục đã xong",T.value.style.width=(n?t/n*100:0)+"%"}function K(){a.value.querySelectorAll(".copybtn").forEach(n=>{n.onclick=t=>{t.stopPropagation(),navigator.clipboard.writeText(n.parentElement.querySelector("code").innerText),n.textContent="đã copy",setTimeout(()=>n.textContent="copy",1200)}})}const N=[];function O(){c.forEach(n=>{n.querySelectorAll("h1,h2,h3,h4,p,li,td,th,pre").forEach(t=>{if(t.querySelector("p,ul,ol,pre,table,li"))return;const o=(t.textContent||"").replace(/\s+/g," ").trim();o.length>3&&N.push({docId:n.id,title:n.dataset.title||"",text:o})})})}function V(){const n=d.value.value.trim().toLowerCase();if(n.length<2){s.value.hidden=!0,l.value.hidden=!1;return}const t=[];for(const e of N)if(e.text.toLowerCase().includes(n)&&(t.push(e),t.length>=60))break;const o=new RegExp("("+n.replace(/[.*+?^${}()|[\]\\]/g,"\\$&")+")","ig");s.value.innerHTML=`<p style="color:var(--muted);font-size:13px">${t.length} kết quả cho “${g(d.value.value)}”</p>`+t.map(e=>`<div class="res" data-id="${e.docId}"><div class="doc">${g(e.title)}</div><div class="line">${g(e.text.slice(0,220)).replace(o,"<mark>$1</mark>")}</div></div>`).join(""),s.value.hidden=!1,l.value.hidden=!0,s.value.querySelectorAll(".res").forEach(e=>{e.onclick=()=>{d.value.value="",s.value.hidden=!0,l.value.hidden=!1,location.hash.slice(1)===e.dataset.id?k(e.dataset.id,null):location.hash=e.dataset.id}})}function F(n){n.key==="/"&&document.activeElement!==d.value&&(n.preventDefault(),d.value.focus()),n.key==="Escape"&&(d.value.value="",s.value.hidden=!0,l.value.hidden=!1,d.value.blur())}function U(){const n=c.filter(t=>t.hidden);n.forEach(t=>t.hidden=!1),window.print(),setTimeout(()=>n.forEach(t=>t.hidden=!0),400)}let f=!1;function G(){f||(f=!0,requestAnimationFrame(()=>{const n=[...a.value.querySelectorAll("section.doc:not([hidden]) h2, section.doc:not([hidden]) h3")];let t=null;for(const o of n)o.getBoundingClientRect().top<120&&(t=o);b.value.querySelectorAll("a").forEach(o=>o.classList.toggle("cur",!!t&&o.getAttribute("href")==="#"+t.id)),f=!1}))}function H(){J(async()=>{const{PACKS:n,countByNote:t}=await import("./packs-BZI_7XGn.js");return{PACKS:n,countByNote:t}},__vite__mapDeps([0,1,2])).then(({PACKS:n,countByNote:t})=>{const o=t(n.ai200);a.value.querySelectorAll("section.doc h2[id], section.doc h3[id]").forEach(e=>{const A=o.get(e.id);if(!A)return;const h=document.createElement("a");h.className="ql",h.href="/ai-200/practice?doc="+encodeURIComponent(e.id),h.target="_blank",h.rel="noopener",h.title=`Luyện ${A} câu hỏi thuộc mục này (mở tab mới)`,h.textContent=`ôn ${A} câu`,e.insertBefore(h,e.querySelector(".anch"))})})}function q(){a.value.style.setProperty("--topbar-h",_.value.offsetHeight+"px")}return W(()=>{c=[...a.value.querySelectorAll("section.doc")],v=c[0].id,O(),H(),w(),K(),I(),z(),q();const n=[[window,"hashchange",z],[window,"resize",q],[window,"scroll",G],[document,"keydown",F]];n.forEach(([t,o,e])=>t.addEventListener(o,e)),D=n.map(([t,o,e])=>()=>t.removeEventListener(o,e))}),$(()=>D.forEach(n=>n())),(n,t)=>{const o=Z("router-link");return tt(),j("div",{ref_key:"rootEl",ref:a,class:"notes-ai"},[i("div",{ref_key:"topbarEl",ref:_,class:"topbar"},[i("button",{class:"btn menu-btn",onClick:t[0]||(t[0]=e=>y.value.classList.toggle("open"))},"☰"),t[5]||(t[5]=i("div",{class:"brand"},[i("span",{class:"dot"}),u("AI-200 "),i("small",null,"Azure AI Cloud Developer Associate · Tiếng Việt")],-1)),t[6]||(t[6]=i("div",{class:"spacer"},null,-1)),i("div",it,[i("input",{ref_key:"searchEl",ref:d,placeholder:"Tìm trong toàn bộ tài liệu… (/)",autocomplete:"off",onInput:V},null,544)]),p(o,{class:"btn",to:"/ai-200/practice",title:"Mở trang luyện trắc nghiệm"},{default:S(()=>[...t[2]||(t[2]=[u("✎ Trắc nghiệm",-1)])]),_:1}),p(o,{class:"btn",to:"/ai-200/labs",title:"Mở trang lab thực hành"},{default:S(()=>[...t[3]||(t[3]=[u("🧪 Thực hành",-1)])]),_:1}),p(nt),i("button",{class:"btn",title:"In / xuất PDF",onClick:U},"⎙"),p(o,{class:"btn",to:"/"},{default:S(()=>[...t[4]||(t[4]=[u("🏠",-1)])]),_:1})],512),i("div",rt,[i("aside",{ref_key:"sideEl",ref:y},[t[7]||(t[7]=i("div",{class:"navtitle"},"Tiến độ học",-1)),i("div",{ref_key:"pcountEl",ref:x,class:"pcount"},"0 / 0 mục đã xong",512),i("div",ct,[i("div",{ref_key:"pbarEl",ref:T},null,512)]),t[8]||(t[8]=i("div",{class:"navtitle"},"Nội dung",-1)),i("nav",{ref_key:"navEl",ref:E},null,512)],512),i("main",null,[i("div",{ref_key:"resultsEl",ref:s,class:"results",hidden:""},null,512),t[1]||(L(-1,!0),(t[1]=i("article",{ref_key:"articleEl",ref:l,innerHTML:X(ot)},null,8,["innerHTML"])).cacheIndex=1,L(1),t[1])]),i("div",{ref_key:"tocEl",ref:b,class:"toc"},null,512)])],512)}}});export{gt as default};
